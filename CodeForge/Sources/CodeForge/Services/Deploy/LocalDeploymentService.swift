...
actor LocalDeploymentService {
    ...
    private let logger = Logger(subsystem: "com.codeforge", category: "LocalDeploymentService")
        ...
        do {
            ...
            case .docker:
                deployment = try await deployDocker(
                    ...
                    imageName: "codeforge-\(safeName)-\(envName)",
                    containerName: "codeforge-\(safeName)-\(envName)",
                    port: port
                )
            case .dockerCompose:
