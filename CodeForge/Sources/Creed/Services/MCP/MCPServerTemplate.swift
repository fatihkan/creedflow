...
struct MCPServerTemplate: Identifiable {
    ...
    static let all: [MCPServerTemplate] = [filesystem, github, promptsChat, creed]
    ...
    static let creed = MCPServerTemplate(
        id: "creed",
        displayName: "Creed",
        description: "Creed project state access for agents",
        icon: "hammer.fill",
        command: "CreedMCPServer",
        defaultArgs: [],
        requiredInputs: [
            RequiredInput(
                id: "binary_path",
                label: "Binary Path",
                placeholder: "/path/to/CreedMCPServer",
                type: .path,
                isCommand: true
            )
        ],
