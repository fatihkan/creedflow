...
/// CLIBackend for Ollama local LLM.
/// Spawns `ollama run <model> "<prompt>"` and collects plain text output.
actor OllamaBackend: CLIBackend {
    nonisolated let backendType = CLIBackendType.ollama
    ...
    init(cliPath: String? = nil) {
        self.cliPath = cliPath
            ?? UserDefaults.standard.string(forKey: "ollamaPath").flatMap({ $0.isEmpty ? nil : $0 })
            ?? Self.findCLI()
    }
    ...
    private var modelName: String {
        UserDefaults.standard.string(forKey: "ollamaModel").flatMap({ $0.isEmpty ? nil : $0 })
            ?? "llama3.2"
    }
    ...
    func execute(_ input: CLITaskInput) async -> (id: UUID, stream: AsyncThrowingStream<CLIOutputEvent, Error>) {
        ...
        process.arguments = ["run", model, fullPrompt]
