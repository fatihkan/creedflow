...
/// CLIBackend for OpenClaw — open-source personal AI assistant agent.
/// Spawns `openclaw agent --message "<prompt>" --format text` and collects output.
actor OpenClawBackend: CLIBackend {
    nonisolated let backendType = CLIBackendType.openclaw
    ...
    init(cliPath: String? = nil) {
        self.cliPath = cliPath
            ?? UserDefaults.standard.string(forKey: "openclawPath").flatMap({ $0.isEmpty ? nil : $0 })
            ?? Self.findCLI()
    }
    ...
    func execute(_ input: CLITaskInput) async -> (id: UUID, stream: AsyncThrowingStream<CLIOutputEvent, Error>) {
        ...
        process.arguments = ["agent", "--message", fullPrompt, "--format", "text"]
