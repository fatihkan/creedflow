...
/// CLIBackend for OpenCode (opencode.ai) — open-source AI coding agent.
/// Spawns `opencode run "<prompt>"` and collects output.
actor OpenCodeBackend: CLIBackend {
    nonisolated let backendType = CLIBackendType.opencode
    ...
    init(cliPath: String? = nil) {
        self.cliPath = cliPath
            ?? UserDefaults.standard.string(forKey: "opencodePath").flatMap({ $0.isEmpty ? nil : $0 })
            ?? Self.findCLI()
    }
    ...
    func execute(_ input: CLITaskInput) async -> (id: UUID, stream: AsyncThrowingStream<CLIOutputEvent, Error>) {
        ...
        process.arguments = ["run", fullPrompt, "--format", "json", "-q"]
