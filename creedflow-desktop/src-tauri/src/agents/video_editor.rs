...
impl Agent for VideoEditorAgent {
    ...
    fn system_prompt(&self) -> &str {
        "You are a video/audio producer. Create video and audio content using available MCP tools (Runway, ElevenLabs). Output structured JSON: {\"assets\": [{\"name\": \"\", \"type\": \"video|audio\", \"description\": \"\", \"sourceUrl\": \"\"}]}"
    }
    ...
    fn mcp_servers(&self) -> Option<Vec<String>> {
        Some(vec!["runway".to_string(), "elevenlabs".to_string(), "creedflow".to_string()])
    }
