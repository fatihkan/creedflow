/// Slack Incoming Webhook — sends notifications at key milestones.
...
impl SlackService {
    ...
    pub async fn notify_task_completion(
        ...
        let msg = format!(
            "✅ Task Completed\n\n\
             ...
             Cost: {}\n\
             Duration: {}",
            task_title, agent_type, backend, cost_str, duration_str,
        );
    ...
    pub async fn notify_task_failure(
        ...
        let msg = format!(
            "❌ Task Failed\n\n\
             ...
             Error: {}\n\
             Retries: {}/{}",
            task_title, agent_type, error, retry_count, max_retries,
        );
        let _ = self.send_message(&msg).await;
    }
    ...
    pub async fn notify_deploy(
        ...
        let msg = format!(
            "{} Deployment {}\n\n\
             ...
             Environment: {}\n\
             Version: {}",
            emoji, status, project_name, environment, version,
        );
        let _ = self.send_message(&msg).await;
    }
    ...
    pub async fn notify_analysis_complete(
        ...
        let msg = format!(
            ...
            project_name, feature_count, task_count,
        );
        let _ = self.send_message(&msg).await;
    }
}
