...
impl CliBackend for LlamaCppBackend {
    ...
    async fn is_available(&self) -> bool {
        self.cli_path.lock().unwrap().is_some() && self.model_path.lock().unwrap().is_some()
    }

    async fn execute(&self, input: TaskInput) -> Result<(Uuid, mpsc::Receiver<OutputEvent>), String> {
        let path = self.cli_path.lock().unwrap().clone().ok_or("llama-cli not found")?;
        let model = self.model_path.lock().unwrap().clone().ok_or("GGUF model path not set")?;
        let id = Uuid::new_v4();
        let (tx, rx) = mpsc::channel(256);

        // llama-cli -m <model> -p "<prompt>" -n 4096 -sys "<system>"
        let mut args = vec![
            "-m".to_string(), model,
